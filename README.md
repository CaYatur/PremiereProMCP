...
|------|-----------------|--------------|
| `sequence_set_in_out` | Real testing walked it through two error messages: first `"sequence.setInPoint is not a function"`, then (after the first fix attempt) `"no candidate method found… Not exposed via UXP Sequence/SequenceEditor"` — the first fix looked for the Action factory on the **wrong object** (`SequenceEditor`) | **Root cause found:** `createSetInPointAction`/`createSetOutPointAction` live on the **Sequence object itself**, not `SequenceEditor` (confirmed against Adobe's official `ppro_reference`, introduced in Premiere 25.6). Now calls `sequence.createSetInPointAction(t)` / `createSetOutPointAction(t)` and commits **both in one compound transaction** (so the timeline never passes through an invalid in>out intermediate state), with the old editor/direct paths kept as fallbacks. Because `clip_append` (a 25.6-era action) works on this build, the build is ≥25.6, so these factories should be present |
...
|------|-------|--------------|
| `track_add` / `track_add_video` / `track_add_audio` | The Premiere UXP API exposes **no** method to add an empty track — verified 2026-07-11 against Adobe's official reference: neither the `Sequence` class nor `SequenceEditor` has any `addTrack`/`addVideoTrack`/`addAudioTrack`/`createAddTrackAction`. This is missing from Premiere itself; no plugin-side code can add it | (1) Create the sequence with enough tracks up front (`sequence_create` / a preset). (2) Place a clip at a higher track index with `clip_overwrite`/`clip_insert` (both now work) — Premiere is expected to auto-create the intervening tracks when it drops the clip there |
| `sequence_create` / `sequence_create_from_media` track count | You get the preset's fixed track count (≈3 video + 3–4 audio) and — because of the `track_add` limitation above — can't add empty tracks afterward | Same workaround: pick a preset with enough tracks, or drop clips onto higher track indices to force-create tracks |
